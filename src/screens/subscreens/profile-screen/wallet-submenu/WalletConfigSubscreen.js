...
export default (props) => {
  ...
  const categoriesIdealPercents = async (oldWalletLocal) => {
    ...
    oldWalletLocal.idealPercents.forEach((oldElement) => {
      listIdealPercents.push({
        idCategory: oldElement.category.idCategory,
        idealPercent: oldElement.idealPercent,
      });
    });
  ...
  const getActualIdealPercent = (index) => {
    if (
      typeof idealPercents === "undefined" ||
      typeof idealPercents[index] === "undefined" ||
      typeof idealPercents[index].idealPercent === "undefined"
    )
      return 0;
    else return idealPercents[index].idealPercent.toFixed(2);
  };

  const getIdealPercentsSum = () => {
    if (typeof idealPercents === "undefined") return 0;
    else {
      let sum = 0;
    ...
    <>
      ...
      <ScrollView keyboardShouldPersistTaps="always">
        ...
        >
          ...
          >
            <Title style={{ color: useTheme().colors.primary }}>
              Porcentagem
            </Title>
            <Text>
              Agora me diga, qual o percentual ideal em cada ativo para você?
            </Text>
            <View
              style={{
                alignItems: "center",
            ...
            >
              {getIdealPercentsSum() != 100 && (
                <Text style={{ color: "#CF3341" }}>
                  O total da soma das categorias deve ser 100%!
                </Text>
              )}
            </View>
            <View>
