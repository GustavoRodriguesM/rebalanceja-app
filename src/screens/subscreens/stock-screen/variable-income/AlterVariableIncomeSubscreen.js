  ...
  const onAlter = async (data) => {
    ...
    if (await new AquisitionService().updateAquisition(obj)) {
      ...
      Toast.show({
        ...
        text2:
          "Não foi possivel alterar o ativo " +
          data.stockSymbolForm.toUpperCase() +
          ". Tente novamente!",
      });
    }
      ...
      >
        <Appbar.Header style={{ alignItems: "center" }}>
          ...
          <Appbar.Content
            ...
            subtitle={"Alterando ".concat(
              props.route.params.alterAquisition.aquisition.stock.symbol
            )}
          />
          <Appbar.Action
            icon="check"
              ...
              {isAutomaticAllocate && (
                ...
                >
                  <Controller
                    name="maxPriceForm"
                    control={control}
                    defaultValue={BRLCurrencyFormat(
                      props.route.params.alterAquisition.aquisition.maxPrice
                    )}
                    rules={{ required: true, min: 0 }}
                    render={({ field: { onChange, onBlur, value } }) => (
                      <TextInput
